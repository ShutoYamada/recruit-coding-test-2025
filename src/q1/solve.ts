...
const parseLine = (line: string): Ticket | null => {
  ...
  // 範囲チェック（parse後すぐにチェック）
  if (startHH > 23) return null;
  if (startMM > 59) return null;
  if (durM > 59) return null;
...
const checkTimeRule = (
  ...
  // Adult が 0 かつ Child を含む場合
  if (hasChildInSet) {
    // 終了が 16:00 (960分) を超える場合、全員NG
    if (endMinutes > 16 * 60) {
      return false;
    }
  }
