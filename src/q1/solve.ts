...
const checkRating = (
  ...
  if (rating === 'PG-12') {
    if (age === 'Child') {
      // A child needs adult in PG-12 movie set
      return hasAdultInSet;
    }
    // Young/Adult can watch PG-12 movie
    return true;
  }
 ...
 * 座席の規則
 * // R18+
 *  - J〜L は Child age === 'Adult'可
 */
const checkSeat = (t: Ticket): boolean => {
  if (t.age !== 'Child') return true;
  const row = t.row.toUpperCase();
  return (row !== 'J' && row !== 'K' && row !== 'L')
};
...
const checkTimeRule = (
  ...
  // If there's at least an adult -> always OK
  if (hasAdultInSet) return true;

  const END_16 = 60 * 16 // 960
  const END_18 = 60 * 18 // 1080
...
const orderReasons = (reasons: string[]): string[] => {
  ...
  if (reasons.includes(MSG.NEED_ADULT)) orders.push(MSG.NEED_ADULT);
  if (reasons.includes(MSG.AGE_LIMIT)) orders.push(MSG.AGE_LIMIT);
  if (reasons.includes(MSG.SEAT_LIMIT)) orders.push(MSG.SEAT_LIMIT);
